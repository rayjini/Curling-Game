...
class SetOfCollisions {
  ...
  handleCollision(aCollision) {
    ...
    let dx = Math.abs(otherStone.getLocation().x - movingStone.getLocation().x) //hor dist between stones
    let dy = Math.abs(otherStone.getLocation().y - movingStone.getLocation().y) //ver dist between stones
    ...
    if (movingStone.getVelocity().vx > 0) { //moving stone is going right
    ...
    if (movingStone.getVelocity().vy > 0) { //moving is going downwards
      if (movingStone.getLocation().y < otherStone.getLocation().y)
        v1y = -v1y
      else v2y = -v2y
    } else {
      //moving stone is moving upwards
      if (movingStone.getLocation().y > otherStone.getLocation().y){
        v2y = -v2y; v1y = -v1y}
      else {v1y = -v1y; v2y = -v2y}
    }
